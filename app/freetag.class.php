...
class freetag {
	 ...
	 */
	var $_table_prefix = 'member_';
	/**
	 * @access private
	 * @var string The regex-style set of characters that are valid for normalized tags.
	 */
	var $_normalized_valid_chars = 'éáűőúöüóíÉÁŰŐÚÖÜÓÍa-zA-Z0-9';
	/**
	 * @access private
	...
	/**
	 * @access private
	 * @var int The minimum length of a tag.
	 */ 
	var $_MIN_TAG_LENGTH = 3;
    /**
	 * @access private
	...
	function get_used_raw_tags($max = 100, $tagger_id = NULL, $offset = 0){
		$db = $this->db;
		...
		$sql = "SELECT raw_tag, COUNT(object_id) AS quantity
			FROM ${prefix}freetags INNER JOIN ${prefix}freetagged_objects
			ON (${prefix}freetags.id = tag_id)
			WHERE 1
			$tagger_sql
		...
		$retarr = array();
		while(!$rs->EOF) {
			$retarr[$rs->fields['raw_tag']] = $rs->fields['quantity'];
			$rs->MoveNext();
		}
